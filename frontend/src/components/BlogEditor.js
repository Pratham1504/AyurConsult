...
const BlogEditor = () => {
    ...
    const [formData, setFormData] = useState({ title: '', description: '', topic: '', youtubeLink: '', productId: '' });
    const [products, setProducts] = useState([]);
    const [isEditMode, setIsEditMode] = useState(false);
    const [loading, setLoading] = useState(true);
    const { setMessage } = useContext(NotificationContext); // Access global message state
    ...
    // Handle Quill editor changes (HTML content)
    const handleDescriptionChange = (content) => {
        setFormData({ ...formData, description: content });
    };

    // Handle form submission (create/edit blog)
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            navigate('/admin/blogs'); // Redirect back to blog list
        } catch (err) {
    ...
    return (
        <div className="blog-editor-page bg-white py-8 px-4 mx-auto max-w-screen-xl lg:py-8 lg:px-6">
            <h1 className="text-2xl text-green-700 font-bold mb-4">{isEditMode ? '| Edit Blog' : '| Create New Blog'}</h1>
